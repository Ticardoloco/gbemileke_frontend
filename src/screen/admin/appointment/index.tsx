...
export default function AdminAppointmentsPage() {
  ...
  // Calculate Today's Total Appointments
  ...
  const handleStatusUpdate = async (
    ...
    toast.promise(updatePromise, {
      loading: `Updating status to ${status.toLowerCase()}...`,
      success: (res) => {
  ...
  const handleDeleteAppointment = async () => {
    ...
    setDeletingId(id);
    const deletePromise = deleteAppointment(id);
    ...
    toast.promise(deletePromise, {
      ...
      error: (err) => {
        return err?.response?.data?.message || "Failed to delete appointment";
      },
      finally: () => {
        setDeletingId(null);
      },
  ...
  const filteredAppointments = appointments.filter((app) => {
    ...
    if (activeTab === "All") {
      ...
      matchesTab = appDateStr === todayStr;
    } else {
      matchesTab = app.status?.toLowerCase() === activeTab.toLowerCase();
    }
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-500">
              Manage patient bookings, monitor daily schedules, and update statuses
            </p>
          </div>

          <div className="flex items-center gap-3">
            {/* Today's Appointments Stat Badge */}
            <button
              onClick={() => setActiveTab("Today")}
              className={`flex items-center gap-2.5 bg-white border px-4 py-2 rounded-xl shadow-sm transition hover:border-emerald-300 ${
                activeTab === "Today" ? "border-emerald-500 ring-2 ring-emerald-500/10" : "border-slate-200"
              }`}
            >
            ...
            <button
              ...
              disabled={loading}
              className="inline-flex items-center gap-2 text-xs font-semibold text-slate-700 bg-white border border-slate-200 hover:bg-slate-50 active:bg-slate-100 px-3.5 py-2.5 rounded-xl transition shadow-sm disabled:opacity-50"
            >
              <RotateCw
        ...
        {!loading && !error && filteredAppointments.length === 0 && (
          <div className="text-center py-16 bg-white rounded-2xl border border-slate-200 shadow-sm px-4">
            ...
            <p className="text-xs text-slate-500 mt-1 max-w-sm mx-auto">
              There are no bookings matching your selected tab or search filter.
            </p>
          </div>
        )}
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {paginatedAppointments.map((app) => {
                ...
                const patientName = app.patient?.fullName || "Anonymous Patient";

                return (
                  <div
                    key={app._id}
                    className="bg-white rounded-2xl border border-slate-200 p-4 sm:p-5 shadow-sm hover:shadow-md transition-all flex flex-col justify-between h-full relative"
                  >
                    <div className="space-y-4">
                      ...
                      <div className="space-y-2 text-xs text-slate-600 bg-slate-50 p-3 rounded-xl border border-slate-100">
                        ...
                        <div className="flex items-center justify-between gap-2">
                          <span className="flex items-center gap-1.5 text-slate-500 shrink-0">
                            <CircleUser className="w-3.5 h-3.5 text-slate-400" /> Gender:
                          </span>
                    ...
                    <div className="mt-5 pt-3 border-t border-slate-100 flex items-center gap-2">
                      {isUpdating ? (
                        <div className="w-full flex items-center justify-center py-2 text-xs font-semibold text-slate-500 gap-2">
                          <Loader2 className="w-4 h-4 animate-spin text-slate-600" /> Updating...
                        </div>
                      ) : (
                        <>
                          {app.status === "Pending" && (
                          ...
                          {(app.status === "Completed" ||
                            ...
                            <span className="text-[11px] font-medium text-slate-400 italic py-1 w-full text-center">
                              No further status actions required
                            </span>
                          )}
                        </>
                      )}
                ...
                {Array.from({ length: totalPages }, (_, idx) => idx + 1).map(
                  (page) => (
                    <button
                      ...
                      className={`min-w-8 h-8 px-2.5 rounded-xl text-xs font-semibold transition ${
                        currentPage === page
                          ? "bg-slate-900 text-white shadow-sm"
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-5 sm:p-6 shadow-xl space-y-4 border border-slate-200 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                <Trash2 className="w-5 h-5 text-rose-600" /> Delete Appointment
              </h3>
              <button
                onClick={() =>
                  setDeleteModal({ isOpen: false, appointmentId: null })
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              Are you sure you want to delete this appointment permanently? This action cannot be undone.
            </p>
            ...
            <div className="flex items-center justify-end gap-2 pt-2">
              ...
              <button
                disabled={deletingId === deleteModal.appointmentId}
                onClick={handleDeleteAppointment}
                className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-semibold bg-rose-600 hover:bg-rose-700 active:bg-rose-800 disabled:opacity-50 text-white rounded-xl transition shadow-sm"
              >
